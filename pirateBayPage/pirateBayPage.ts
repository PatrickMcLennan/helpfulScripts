...
const pirateBayPage = async (userName: string): Promise<ITorrent[]> => {
    ...
    const videoResults: ITorrent[] = await newPage.evaluate((): ITorrent[] => {
        ...
        return allRows.map((singleResult: HTMLTableRowElement): ITorrent => {
            return {
                ...
                tags: [...singleResult.querySelectorAll('.vertTh center a')].map((aElement: HTMLAnchorElement): string => aElement.textContent),
