...
const youtubeDl = async (videoUrl: string, directory: string) => {
    ...
    script.on('error', (err: Error): void => logger(`There was an error exectuing youtube-dl on ${timeStamp()} -> ${videoUrl}, ${err}`, directory));
    ...
    return script.on('exit', (code: number, signal: string): void => {
        if (code !== 0) {
            logger(`There was an error exectuing youtube-dl on ${timeStamp()} -> ${videoUrl}`, directory);
        }
    });
};
